...
worker.addEventListener('activate', function (event) {
    ...
    var expectedCacheNamesSet = new Set(Object.keys(CURRENT_CACHES).map(function (k) { return CURRENT_CACHES[k]; }));
...
worker.addEventListener('fetch', function (event) {
    ...
    if (event.request.url.match("mapperData.json")) {
        event.respondWith(caches.open(CURRENT_CACHES.maps).then(function (cache) {
